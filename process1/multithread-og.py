...
client_id = "0fff9968cf5b43dd9e05b114645dd27d" #"f3e67b4ead6e426bb342191e25b268f2" #"8dabf54c571d48dbbd6e6eef9d714609" # #"9d7a7cb432794f1e97638a9b5af8cb5e" #"48c55dd9e076465db2cb6a0e8b18c9e3"
client_secret = "eaeedb8dfe9d427d89678af6d9fdb822" #"6d725ee38db442a2938270da4cd4fa86" #"fa5897bce6dd46dab66dd5e8d194110a" # #"12b8f282880f4164a9816dd4e9d1aed3" #"41e233bac46d4d0992c9846c8fab6c31"
...
CLIENT_ACCESS_TOKEN = "aFQS96sWg23h9nDdQsVZJBsJ7cG7e9fZFK14-ruuQ88oMeL28-45dPzxNfXycwgk" #"94UDqOPc35juXwk8Tdjx4RgPowEHgIT1skJCHvJ3OufclVb4gd0R-U4KI9f4zmm3" #"-bFmB_1NeOCHxvd21MGlWp0ntrM95eseLbPWnYs3SnZcXIa4wf2JmQX0lFlIby9r" #"94UDqOPc35juXwk8Tdjx4RgPowEHgIT1skJCHvJ3OufclVb4gd0R-U4KI9f4zmm3" 
...
for name in artist_names[533:]:
    ...
    #get_artist_albums(artist_uri).to_csv("ArtistDetails.csv", index=False)
...
client_secret = "fa5897bce6dd46dab66dd5e8d194110a"
...
def get_artist_albums(artist):
    ...
    start = time.time()
    with concurrent.futures.ThreadPoolExecutor() as executer:
        futures = [executer.submit(album_helper, (all_data), (unique), (album)) for album in albums]
        return_value = [future.result() for future in futures]
        all_data = return_value
    print("Get Album Threading Elapsed Time: %s" % (time.time() - start))
    # for album in albums:
    #     all_data = album_helper(all_data, unique, album)
    return all_data

def album_helper(all_data, unique, album):
    name = album['name'].lower()
    if name not in unique:
        logger.info('ALBUM: %s', name)
        unique.add(name)
...
artist_names = open("/Users/mariamtamer/Desktop/folder/process1/artists.txt").read().splitlines()
#artis_names = artist_names[0:2] #testing case


for name in artist_names[64:250]:
    artist_uri = get_artist_uri(name)
    #artist = get_artist_info(artist_uri)
    #print(get_artist_albums(artist_uri))
    #get_artist_albums(artist_uri).to_csv("Songs.csv", index=False)
